...
#   geom_polygon(data = danger_map,
#                aes(x = long, y = lat, group = group, fill = MAKE)) +
#   coord_map()
...
deaths <- acc %>%group_by(COUNTY) %>% summarise(death_sum = sum(FATALS))

death_map <- deaths %>% left_join(glcs %>% select(c('County Name', 'County Code')), by = c('COUNTY' = 'County Code')) %>% mutate('County Name' = tolower('County Name')) %>% unique() %>%
  left_join(counties, by = c('County Name' = 'subregion'))

ggplot(counties, aes(x = long, y = lat)) + 
  geom_polygon(aes(group = group)) +
  geom_polygon(data = death_map,
               aes(x = long, y = lat, group = group, fill = death_sum)) +
  coord_map()
